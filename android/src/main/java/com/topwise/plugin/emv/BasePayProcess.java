...
abstract class BasePayProcess {
        ...
        try {
            if (ret == PayDataUtil.EMV_OK) {
                ...
                if (ret == PayDataUtil.CLSS_RESELECT_APP) {
                    ret = TopUsdkManager.entry.delCandListCurApp();
                    Log.d(TAG, "delCandListCurApp ret: " + ret);
                    if (ret == PayDataUtil.EMV_OK) {
                ...
                } else if (ret == PayDataUtil.ICC_CMD_ERR) {
    ...
    EmvCapk getCurrentCapk(byte[] rid, byte[] index) {
        Log.d(TAG, "getCurrentCapk rid: " + BytesUtil.bytes2HexString(rid));
        Log.d(TAG, "getCurrentCapk index: " + BytesUtil.bytes2HexString(index));
...
//            }
            ...
            Log.d(TAG, "return emvCapk: " + emvCapk.toString());
            return emvCapk;
        } else {
            Log.d(TAG, "capk is null !!!");
        }
    ...
    byte[] getCurrentAidData(String aidHex) {
        Log.d(TAG, "getCurrentAidData() aidHex: " + aidHex);
        Aid aid = db.getAidDao().findByAidAndAsi(aidHex);
        Log.d(TAG, "aid:" + aid.toString());
        byte[] aidData = null;
        if (aid != null) {
            aidData = aid.getTlvList().getBytes();
        }
        Log.d(TAG, "aidData: " + BytesUtil.bytes2HexString(aidData));
        return aidData;
    }
}
