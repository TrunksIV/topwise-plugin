 ...
 */
public class MirProcess extends BasePayProcess {
    private static final String TAG = MirProcess.class.getSimpleName();
    @Override
    public void startPay(Bundle param, TransResultListener listener) {
        Log.d(TAG, "startPay");
        try {
            //init
            TopUsdkManager.mir.initialize();
            ...
            int res = TopUsdkManager.mir.setFinalSelectData(data, len,null);
            ...
            listener.setKernalData(PayDataUtil.KERNTYPE_MIR, aidData);
            //读取终端信息
            ...
            if (terminalInfo != null) {
                ...
                if (!list.getList().isEmpty()) {
                    ...
                    TopUsdkManager.mir.setTLVDataList(tlvData, tlvData.length);
                }
            }
            ...
            Log.d(TAG, "gpoProc=========: " );
            res = TopUsdkManager.mir.gpoProc();
            Log.d(TAG, "gpoProc res: " + res);
            if (!handleResult(res, listener)) {
                return;
            }
            Log.d(TAG, "readData=========: " );
            res = TopUsdkManager.mir.readData();
            ...
            int res1 = TopUsdkManager.mir.getTLVDataList(BytesUtil.hexString2Bytes("57"),
            ...
            res = TopUsdkManager.mir.startTrans((byte)0);
            Log.d(TAG, "startTrans res: " + res);
            if (!handleResult(res, listener)) {
                return;
            }
            Log.d(TAG, "cardAuth=========: " );
            res = TopUsdkManager.mir.cardAuth();
            ...
            res = TopUsdkManager.mir.getTLVDataList(new byte[]{(byte) 0xDF, (byte) 0x81, 0x29},
    ...
    private String getCurrentRid() {
        ...
        try {
            //delete
            TopUsdkManager.mir.delAllRevocList();
            TopUsdkManager.mir.delAllCAPK();
            int[] realLen = new int[1];
            byte[] aucAid = new byte[17];
            int res = TopUsdkManager.mir.getTLVDataList(new byte[]{0x4F}, 1,
    ...
    private void addCapk(String aid) {
        try {
            ...
            int[] realLen = new int[1];
            Log.d(TAG, "aid: " + aid);
            int res = TopUsdkManager.mir.getTLVDataList(new byte[]{(byte) 0x8F},
                    1, 1, index, realLen);
            if (res == PayDataUtil.EMV_OK) {
                ...
                if (emvCapk != null) {
                    res = TopUsdkManager.mir.addCAPK(emvCapk);
